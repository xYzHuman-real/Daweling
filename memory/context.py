...
class ContextBundle:
    """Relevant project memories and prior experiences for a workflow step."""
    goal: Goal
    memories: list[MemoryEntry]
    learning: LearningContext
    def as_dict(self) -> dict[str, Any]:
        return {"goal": self.goal.description, "goal_context": self.goal.context, "memories": [{"key": e.key, "value": e.value, "category": e.category, "created_at": e.created_at} for e in self.memories], "experiences": [e.value for e in self.learning.experiences]}
    def as_prompt_context(self) -> str:
        ...
        return "\n\n".join((memory_section, self.learning.as_prompt_context()))
